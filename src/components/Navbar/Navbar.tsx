...
const Navbar: React.FC = () => {
  ...
  return (
    ...
    '>
      {/* left box  */}
      <div onClick={()=>router('/')} className='h-full flex justify-center items-center gap-2'>
        <img 
          className='w-[40px] h-[40px] rounded-full'
          src="./assets/img12.png" 
          alt="Attendance Logo" 
        />
        <span className="leading-[20px] sm:text-lg font-title text-[20px]">Attendance</span>
      </div>

      {
        ...
        (
          <div className='h-full flex gap-2 justify-center items-center text-xs'>
            <button onClick={()=>router('/login')} className='px-2 py-3 bg-[#161D29] rounded-lg text-xs'>Login</button>
            <button onClick={()=>router('/signup')} className='px-2 py-3 bg-[#161D29] rounded-lg text-xs'>Sign up</button>
          </div>)
        :
        (
